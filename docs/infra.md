...
|---|---|---|---|---|---|
| **Cerberus** | Edge / infra node | Arch Linux (headless) | 10.10.10.1 (hub) | 192.168.X.251 (static) | `ssh cerberus` or `ssh -p 2121 operator@192.168.X.251` |
| **NightForge** | Operator workstation | Arch Linux | 10.10.10.3 | 192.168.X.156 (DHCP) | `ssh nightforge` |
| **Tairn** | Attack node / C2 | NixOS 24.11 | 10.10.10.4 | 192.168.X.122.230 (libvirt NAT) | `ssh tairn` (WireGuard only) |
| **Hermes** | Redirector VM | Alpine Linux 3.23.3 | 10.10.10.5 | 192.168.X.122.200 (static, libvirt NAT) | `ssh operator@10.10.10.5` (WireGuard only) |
...
- NightForge SSH config uses 10.10.10.4 (WireGuard), not 192.168.X.122.230 (libvirt NAT)
- Cerberus cannot reach 192.168.X.122.x -- libvirt NAT is only reachable from NightForge
- All spokes must have AllowedIPs = 10.10.10.0/24 for Cerberus peer, not 10.10.10.1/32
...
| 8489 | CR1MS0N-ops-dashboard | 10.10.10.3:9090 via WireGuard |
...
|---|---|---|---|---|
| default | virbr0 | 192.168.X.122.0/24 | NAT | Tairn, Hermes VMs |
